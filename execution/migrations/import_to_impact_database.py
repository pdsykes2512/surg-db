#!/usr/bin/env python3
"""
Import CSV data into NEW 'impact' database for quality comparison.
Based on import_fresh_with_improvements.py but targets 'impact' database.
"""
...
# Load secrets from /etc/impact/secrets.env
load_dotenv('/etc/impact/secrets.env')

class ImpactDatabaseImporter:
    def __init__(self, db_name: str = "impact"):
        ...
        for coll in ['patients', 'episodes', 'treatments', 'tumours']:
            ...
            if count > 0:
                print(f"  Removing {count} documents from {coll}")
    ...
    def parse_date(self, date_val) -> Optional[str]:
        ...
        for fmt in formats:
            try:
                ...
                # Fix 2-digit year - be more conservative
                if dt.year > 2050 or dt.year > datetime.now().year:
    ...
    def import_patients(self, patient_csv: str):
        ...
        print(f"Found {len(df)} patient records in CSV")
        ...
        for idx, row in df.iterrows():
            ...
            # Parse DOB
            dob = self.parse_dob(row.get('P_DOB'))
            ...
            patient_doc = {
                ...
                'hospital_number': str(row.get('Hosp_No', '')).strip() or None,
                'demographics': {
            ...
            # Build mappings - Use Hosp_No for linking to surgeries/tumours
    ...
    def import_surgeries(self, surgery_csv: str):
        """Import surgeries as episodes and treatments with lead clinician"""
        print("\n=== Importing Surgeries ===")
        df = pd.read_csv(surgery_csv, low_memory=False)
        print(f"Found {len(df)} surgery records in CSV")
        ...
        for idx, row in df.iterrows():
            ...
            # Use Hosp_No → patient_id mapping
            ...
            referral_date = self.parse_date(row.get('DateRefS'))
            ...
            # Extract surgeon from CSV for lead_clinician
            ...
            if surgery_date:
                ...
                treatment_doc = {
                    ...
                    'surgery_type': str(row.get('ProcName', '')).strip() or None,
                    'approach': str(row.get('ModeOp', '')).strip() or None,
                    'urgency': str(row.get('ASA', '')).strip().lower() or None,
    ...
    def import_tumours(self, tumour_csv: str):
        ...
        print(f"Found {len(df)} tumour records in CSV")
        ...
        for idx, row in df.iterrows():
            ...
            tumour_doc = {
                ...
                'patient_id': patient_id,
                'site': str(row.get('TumSite', '')).strip() or None,
...
def main():
    print("="*60)
    print("IMPORTING DATA INTO 'IMPACT' DATABASE")
    print("="*60)
    print()

    importer = ImpactDatabaseImporter(db_name='impact')

    # Import in order
    print("Using CSV files from /root/.tmp/")
    ...
    print("\n✓ Import complete! Data is in 'impact' database.")
